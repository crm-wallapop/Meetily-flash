...
mod tests {
    ...
    /// Task 5.1 (LOAD-BEARING) — extractor-only parity, UNCONDITIONAL on
    /// committed fixtures. Same boundary layout (one chunk per fixture clip,
    /// in matched order); only the extractor differs: the sherpa reference
    /// embeddings (`tests/fixtures/nemo_c5_reference_embeddings.json`,
    /// captured out-of-process via embed-probe-sherpa because the two ORT
    /// runtimes cannot coexist in-process) vs the ported extractor's
    /// (`nemo_c5_port_embeddings.json`, generated by the identical
    /// slice+resample pipeline). AHC at the production merge threshold must
    /// produce IDENTICAL cluster partitions — the cosine gate proves
    /// per-clip fidelity; this proves the downstream clustering DECISIONS
    /// are unchanged.
    #[test]
    fn extractor_only_parity_unconditional() {
        let fixtures_dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures");
        let load = |name: &str| -> serde_json::Value {
            serde_json::from_str(
                &std::fs::read_to_string(fixtures_dir.join(name))
                    .unwrap_or_else(|e| panic!("committed fixture {} is REQUIRED: {}", name, e)),
            )
            .expect("json")
        };
        let sherpa = load("nemo_c5_reference_embeddings.json");
        let port = load("nemo_c5_port_embeddings.json");

        // sherpa side: sets → clips; port side: flat clips with set+id.
        let mut sherpa_by_key: std::collections::HashMap<String, Vec<f32>> =
            std::collections::HashMap::new();
        for (_set_name, clips) in sherpa["sets"].as_object().expect("sets object") {
            for clip in clips.as_array().expect("arr") {
                if clip.get("skipped").and_then(serde_json::Value::as_bool).unwrap_or(false) {
                    continue;
                }
                if let (Some(id), Some(emb)) = (
                    clip.get("id").and_then(serde_json::Value::as_str),
                    clip.get("embedding").and_then(serde_json::Value::as_array),
                ) {
                    if !emb.is_empty() {
                        sherpa_by_key.insert(
                            format!("{}/{}", _set_name, id),
                            emb.iter().map(|x| x.as_f64().expect("f") as f32).collect(),
                        );
                    }
        ...
        let mut port_by_key: std::collections::HashMap<String, Vec<f32>> =
            std::collections::HashMap::new();
        for clip in port["clips"].as_array().expect("clips array") {
            if clip.get("skipped").and_then(serde_json::Value::as_bool).unwrap_or(false) {
                continue;
            }
            let key = format!(
                "{}/{}",
                clip.get("set").and_then(serde_json::Value::as_str).expect("set"),
                clip.get("id").and_then(serde_json::Value::as_str).expect("id")
            );
            let emb: Vec<f32> = clip["embedding"]
                .as_array()
                .expect("emb arr")
                .iter()
                ...
                .collect();
            if !emb.is_empty() {
                port_by_key.insert(key, emb);
            }
        }

        // Matched clip keys embedded on BOTH sides, in stable order.
        let mut keys: Vec<&String> = sherpa_by_key.keys().filter(|k| port_by_key.contains_key(*k)).collect();
        keys.sort();
        assert!(
            keys.len() >= 8,
            "need >=8 matched embedded clips for a meaningful partition comparison"
        );

        let mk_chunks = |side: &std::collections::HashMap<String, Vec<f32>>| -> Vec<Chunk> {
            keys.iter()
                .enumerate()
                .map(|(i, k)| Chunk {
                    start_sample: i * 32000,
                    end_sample: i * 32000 + 32000,
                    duration_secs: 2.0,
                    embedding: side[*k].clone(),
                })
                .collect()
        };
        let (labels_sherpa, _) = cluster_by_centroids(&mk_chunks(&sherpa_by_key), 0.40);
        let (labels_port, _) = cluster_by_centroids(&mk_chunks(&port_by_key), 0.40);
        // Identical partition shape (labels may renumber; canonicalize).
        let canon = |labels: &[u32]| -> Vec<usize> {
            let mut next = 0usize;
            let mut map = std::collections::HashMap::new();
            labels
        ...
        eprintln!(
            "EXTRACTOR-PARITY: {} matched clips, clusters sherpa={} port={}",
            keys.len(),
            labels_sherpa.iter().collect::<std::collections::HashSet<_>>().len(),
            labels_port.iter().collect::<std::collections::HashSet<_>>().len(),
        );
        assert_eq!(
            canon(&labels_sherpa),
            canon(&labels_port),
            "extractor swap changed AHC clustering decisions — the port is NOT decision-equivalent"
        );
    }
