// Enhance dialog model selection is Whisper-only.
//
// WHY: Enhance is the diarization boundary source. Only Whisper rows carry
// token timestamps, so alignment can split text word-exactly at speaker
// changes via align_with_tokens. User decision 2026-08-27: Parakeet was
// removed as an engine entirely (config migrated in the backend), and the
// dialog's fallback between available Whisper models follows quality order.
...
test.describe('enhance model default (whisper-only)', () => {
  ...
  test('offers no Parakeet entries even with a legacy parakeet config', async ({ page }) => {
    ...
    // Override AFTER the defaults fixture: simulate a pre-migration DB row
    // that still names Parakeet. The UI must not surface it anywhere.
    ...
    const combo = page.getByRole('combobox').filter({ hasText: /Whisper:/ });
    await expect(combo).toBeVisible({ timeout: 10_000 });
    await expect(combo).toContainText(/Whisper:/, { timeout: 10_000 });
    // No Parakeet model may appear even though the (stale) backend would serve one.
    await expect(combo).not.toContainText(/Parakeet/i);
  });
