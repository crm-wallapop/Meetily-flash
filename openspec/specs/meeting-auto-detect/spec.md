...
> **Invariant: notify_exit resets connection_first_seen_at**
> The adapter SHALL reset `connection_first_seen_at` to `None` in `notify_exit()` on the `InCall → Idle` transition. This ensures that each new call begins with a clean pre-existing-check state, and that app restarts with stale WASAPI sessions do not bypass the `not_preexisting` guard. (This is the load-bearing reason the oscillation loop was 20s — a stale timestamp > detector_start caused immediate re-entry into InCall after MeetingEnded.)
