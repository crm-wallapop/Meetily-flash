# Onboarding — Capability Spec

## Purpose

Governs persistence of first-run onboarding state across app launches. Once the user completes setup, the onboarding store's `completed: true` MUST be read faithfully on every subsequent launch and SHALL NOT be overwritten by the client's auto-save path. The client status-load path sets the persisted `completed` value into React state before any slow model-verification probes, so the existing completion guard blocks debounced auto-saves from clobbering real state with mount-time defaults.
