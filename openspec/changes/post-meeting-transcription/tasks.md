...
- [ ] 2.1 Write a failing test `incremental_saver_writes_no_ckpt_files`: assert that during a recording the only files written to the meeting folder are `audio.mp4` (in progress) and `metadata.json`
- [ ] 2.2 Remove `.ckpt` writes from `incremental_saver.rs`; the saver streams directly to MP4 without intermediate checkpoint files
