...
- **[SQLite host-param ceiling]** The implementation uses per-row INSERTs (15 host params each), so no single statement approaches the 999-host-param ceiling regardless of N. The original design proposed multi-row VALUES with chunking at the ceiling; this was simplified to per-row inserts because the per-statement bound is constant (~15) and well under the ceiling, so no chunking is needed. A large-N test (N=120) guards the path.
