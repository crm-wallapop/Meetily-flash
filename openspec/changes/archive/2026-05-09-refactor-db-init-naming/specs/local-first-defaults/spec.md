...
- **THEN** a settings row with id=`'1'`, provider=`'ollama'`, model=`'qwen3.5:4b'`, whisper_model=`'large-v3-turbo'` SHALL exist after `_seed_defaults()` completes
...
- **THEN** the existing row SHALL remain unchanged after `_seed_defaults()` completes
