...
- **B3 spike outcome:** does `env_logger` write through inherited stdio under the GUI subsystem? **Resolved 2026-06-28 (confirmed):** Yes. Built the dev bin with the `not(debug_assertions)` guard dropped (GUI subsystem in debug) and ran it standalone with `RUST_LOG=debug`; the startup logs streamed to the launching shell via inherited stderr — `Starting application...`, `Application setup complete`, and `AUMID branded for dev-build toasts` all appeared (169 log lines captured). No `AttachConsole` fallback needed; the preferred path (drop the guard) is the shipped fix. The `meetily://` console-flash elimination is architecturally guaranteed by the subsystem attribute itself — a GUI-subsystem process cannot allocate a console, so the single-instance secondary that forwards each activation no longer flashes. A live visual confirm of the no-flash is a user item, not a blocker (the `__dev_inject_deep_link` seam is in-process and does not reproduce the secondary-process path).
