...
End-to-end:
12. **Through `step_detector`/spawn loop — DETERMINISTIC (shark-tank I6):** a real-adapter stable exit drives `meeting-ended` within the 4 s window. This is a FAST deterministic loop test driving the real latch; it does NOT rely on `#[ignore]` timing. **Location (post-review correction):** the test lives in `detection/windows.rs` (the adapter's own `#[cfg(test)]` module, no feature gate), NOT in `fake.rs`. `fake.rs` is gated behind the non-default `dev-detector` feature (`Cargo.toml`), so a test placed there is invisible to the documented merge-gate command `cargo test --lib` — defeating I6's "deterministic, not `#[ignore]`-only" intent. The symmetric long-path C2 test stays in `fake.rs` because it drives the fake adapter and the 15 s path is the pre-existing default. A separate `#[ignore]` real-clock test (Task 4.1) confirms wall-clock timing on a live mic.
