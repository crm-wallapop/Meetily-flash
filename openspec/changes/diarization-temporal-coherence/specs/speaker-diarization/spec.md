...
The smoothing pass SHALL perform neighborhood-voted re-assignment: for each chunk, the system SHALL compute, for each candidate label k, a vote `score(k) = Σ cos(e_j, centroid_k) · decay(|i−j|)` summed over the chunk's ±W temporal neighbors (default W = 3), where `e_j` is the NEIGHBOR's embedding (not the chunk's own — using the chunk's own embedding would reduce the vote to nearest-centroid and fix nothing). The system SHALL reassign the chunk's label only when the winning label's score exceeds the current label's score by a positive confidence margin, so that on a clean, high-confidence input no chunk flips (the pass is a near-no-op). The system SHALL then recompute duration-weighted centroids from the cleaned labels and iterate the re-assign/recompute cycle up to a fixed cap (default 2 iterations) so that recovered chunks refine the centroids used in the next pass.

After the iteration, the system SHALL merge a same-label run shorter than `MIN_SMOOTH_SEGMENT_SECS` (default ~10 s) into a neighbor ONLY when both adjacent runs share the same label as each other (a flicker island) OR the short run's embedding is closer to one neighbor by a clear acoustic margin. The system SHALL NOT merge a short run sandwiched between two different speakers (a genuine interjection).
